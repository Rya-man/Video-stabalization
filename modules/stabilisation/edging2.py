...
def add_image_padding(frame, padding_size, cover_image):
    # Add reflection padding first to avoid edge issues
    padded_frame = cv2.copyMakeBorder(
        frame, padding_size, padding_size, padding_size, padding_size, cv2.BORDER_REFLECT
    )

    # Overlay the cover image on the reflection-padded area
    padded_frame[0:padding_size, :] = cover_image[0:padding_size, :]   # Top
    padded_frame[-padding_size:, :] = cover_image[-padding_size:, :]   # Bottom
    padded_frame[:, 0:padding_size] = cover_image[:, 0:padding_size]   # Left
    padded_frame[:, -padding_size:] = cover_image[:, -padding_size:]   # Right
...
def stabilize_frame(prev_frame, curr_frame, prev_transform, padding_size, cover_image):
    ...
    padded_frame = add_image_padding(curr_frame, padding_size, cover_image)
...
def stabilize_video(input_video_path, output_video_path, cover_image_path, padding_size=50):
    clip = VideoFileClip(input_video_path)

    # Load the cover image and resize it to cover the padding area
    cover_image = cv2.imread(cover_image_path)
    cover_image = cv2.resize(cover_image, (clip.size[0] + 2 * padding_size, clip.size[1] + 2 * padding_size))

    # Use the first frame as the background model
    first_frame = next(clip.iter_frames())
    background_frame = add_image_padding(first_frame, padding_size, cover_image)
    ...
    for i, frame in enumerate(clip.iter_frames()):
        ...
        stabilized_frame, prev_transform = stabilize_frame(prev_frame, frame, prev_transform, padding_size, cover_image)
...
# Define the input and output video paths and the cover image path
input_video_path = r"D:\projects\video_stabl\testy.mp4"
output_video_path = 'filler_with_image_cover.mp4'
cover_image_path = r"bg.jpeg"

# Stabilize the video
stabilize_video(input_video_path, output_video_path, cover_image_path)
